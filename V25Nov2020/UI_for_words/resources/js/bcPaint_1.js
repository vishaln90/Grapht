	...
	$.extend(true, $.fn.bcPaint1, {
		...
		insert_into_db : function() {
				...
				jQuery.each(x_start, function( i, val ) {
					...
					x_start_perc = parseFloat(x_start[i])*100/parseFloat(width_canvas); 
						if (x_start_perc < 4) { x_start_perc=0};
					x_end_perc = parseFloat(x_end[i])*100/parseFloat(width_canvas); 
						if (x_end_perc > 96) { x_end_perc=100};
				...
				$.ajax({url: 'http://localhost:3000/words/evaluate_words_to_chars',
					type: 'PUT',
		...
		**/
		onMouseUp : function(e) {
			...
			var line_text = $('#bcPaint1').find('#word_text').val();
			//alert(line_text);
			line_text = line_text.replace("  "," ");
			line_text = line_text.split(" ");
		if((jQuery.type(line_text[counter.length-1]) == "undefined" || line_text[counter.length-1] == "" ) && x_start.length <1)
				{alert("Please enter this line in input box correctly-1."); word_value.push("dummy_value");$.fn.bcPaint1.undo_function();}
			else
				{
