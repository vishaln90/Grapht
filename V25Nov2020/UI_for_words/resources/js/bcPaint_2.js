(function( $ ) {
	...
	**/
	$.fn.bcPaint2 = function (options) {
		...
		//$('.bcPaint-image2').css("background-image","url('Image Storage/Sample2/Lines/Line1.jpg')");
		...
		return this.each(function () {
			...
			//$('#bcPaintCanvas2').css("width","100%");
			//$('#bcPaintCanvas2').css("height","100%");
			...
			//width_canvas = $('.bcPaint-image2').width();//paintCanvas.width;
			//height_canvas = $('.bcPaint-image2').height();//paintCanvas.height;
			width_canvas = paintCanvas.width;
			height_canvas = paintCanvas.height;
			//alert($('#bcPaint2').find('#bcPaint-canvas-container').width() + " vs " + width_canvas);
			//alert(height_canvas);
	...
	$.extend(true, $.fn.bcPaint2, {
		...
		*/
		...
		get_sample : function() {	
					...
					$.get("http://localhost:3000/lines", function(data){
						...
						//alert(line_image_name);
						$('#bcPaintCanvas2').css("background-image","url('//" + line_image_path + line_image_name + "')");
						$('#bcPaintCanvas2').css("background-size","100% 100%");
						$.fn.bcPaint2.update_line_status("In-progress");
		...
		insert_into_db : function() {
				...
				$.fn.bcPaint2.update_line_status("Complete");
		},
		...
		**/
		onMouseUp : function(e) {
			...
			var line_text = $('#bcPaint2').find('#word_text').val();
			//alert(line_text);
			line_text = line_text.replace("  "," ");
			line_text = line_text.split(" ");
		if((jQuery.type(line_text[counter.length-1]) == "undefined" || line_text[counter.length-1] == "" ) && x_start.length <1)
				{alert("Please enter this line in input box correctly-1."); word_value.push("dummy_value");$.fn.bcPaint2.undo_function();}
			else
				{
					word_value.push(line_text[counter.length-1]);
					if (endPoint.x-startPoint.x < 0.01 * width_canvas || endPoint.y-startPoint.y < 0.01 * height_canvas) {$.fn.bcPaint2.undo_function();}
					if (word_value[counter.length-1] == "" || counter.length > line_text.length) 
						{alert("Please ensure that the line entered in the input box is correct.");$.fn.bcPaint2.undo_function();}
		...
		outline_blocks : function() {
			$.fn.bcPaint2.clearCanvas();
			$('#bcPaint2').find('#bcPaint-mid-right').text("");
			var current_text ="";
