(function( $ ) {
	...
	**/
	$.fn.bcPaint3 = function (options) {
		...
		//$('.bcPaint-image3').css("background-image","url('Image Storage/Sample2/Lines/Line1.jpg')");
		...
		return this.each(function () {
			...
			//$('#bcPaintCanvas3').css("width","100%");
			//$('#bcPaintCanvas3').css("height","100%");
			...
			//width_canvas = $('.bcPaint-image3').width();//paintCanvas.width;
			//height_canvas = $('.bcPaint-image3').height();//paintCanvas.height;
			width_canvas = paintCanvas.width;
			height_canvas = paintCanvas.height;
			//alert($('#bcPaint3').find('#bcPaint-canvas-container').width() + " vs " + width_canvas);
			//alert(height_canvas);
	...
	$.extend(true, $.fn.bcPaint3, {
		...
		*/
		...
		get_sample : function() {	
					...
					$.get("http://localhost:3000/lines", function(data){
						...
						$('#bcPaintCanvas3').css("background-image","url('//" + line_image_path + line_image_name + "')");
						$('#bcPaintCanvas3').css("background-size","100% 100%");
						$.fn.bcPaint3.update_line_status("In-progress");
		...
		insert_into_db : function() {
				...
				$.fn.bcPaint3.update_line_status("Complete");
		},
		...
		**/
		onMouseUp : function(e) {
			...
			var line_text = $('#bcPaint3').find('#word_text').val();
			//alert(line_text);
			line_text = line_text.replace("  "," ");
			line_text = line_text.split(" ");
		if((jQuery.type(line_text[counter.length-1]) == "undefined" || line_text[counter.length-1] == "" ) && x_start.length <1)
				{alert("Please enter this line in input box correctly-1."); word_value.push("dummy_value");$.fn.bcPaint3.undo_function();}
			else
				{
					word_value.push(line_text[counter.length-1]);
					if (endPoint.x-startPoint.x < 0.01 * width_canvas || endPoint.y-startPoint.y < 0.01 * height_canvas) {$.fn.bcPaint3.undo_function();}
					if (word_value[counter.length-1] == "" || counter.length > line_text.length) 
						{alert("Please ensure that the line entered in the input box is correct.");$.fn.bcPaint3.undo_function();}
		...
		outline_blocks : function() {
			$.fn.bcPaint3.clearCanvas();
			$('#bcPaint3').find('#bcPaint-mid-right').text("");
			var current_text ="";
