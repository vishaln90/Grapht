(function( $ ) {
	...
	**/
	$.fn.bcPaint4 = function (options) {
		
		//$('.bcPaint-image4').css("background-image","url('Image Storage/Sample2/Lines/Line1.jpg')");
		...
		return this.each(function () {
			var rootElement 	= $(this),
				colorSet		= $.extend({}, $.fn.bcPaint4.defaults, options),
			...
			var bcCanvas = rootElement.find('#bcPaintCanvas4');
			...
			//$('#bcPaintCanvas4').css("width","100%");
			//$('#bcPaintCanvas4').css("height","100%");
			// get canvas pane context
			paintCanvas = document.getElementById('bcPaintCanvas4');
			...
			//width_canvas = $('.bcPaint-image4').width();//paintCanvas.width;
			//height_canvas = $('.bcPaint-image4').height();//paintCanvas.height;
			width_canvas = paintCanvas.width;
			height_canvas = paintCanvas.height;
			//alert($('#bcPaint4').find('#bcPaint-canvas-container').width() + " vs " + width_canvas);
			//alert(height_canvas);

			// bind mouse actions
			paintCanvas.onmousedown = $.fn.bcPaint4.onMouseDown;
			paintCanvas.onmouseup = $.fn.bcPaint4.onMouseUp;
			paintCanvas.onmousemove = $.fn.bcPaint4.onMouseMove;

			// bind touch actions
			paintCanvas.addEventListener('touchstart', function(e){
				$.fn.bcPaint4.dispatchMouseEvent(e, 'mousedown');
			});
			paintCanvas.addEventListener('touchend', function(e){
  				$.fn.bcPaint4.dispatchMouseEvent(e, 'mouseup');
			});
			paintCanvas.addEventListener('touchmove', function(e){
				$.fn.bcPaint4.dispatchMouseEvent(e, 'mousemove');
			});
	...
	$.extend(true, $.fn.bcPaint4, {
		...
		*/
		...
		get_sample : function() {	
					...
					$.get("http://localhost:3000/lines", function(data){
						...
						$('#bcPaintCanvas4').css("background-image","url('//" + line_image_path + line_image_name + "')");
						$('#bcPaintCanvas4').css("background-size","100% 100%");
						$.fn.bcPaint4.update_line_status("In-progress");
						var available_frame = $('#current_frame_count').text();
						//alert("2>"+available_frame);
		...
		insert_into_db : function() {
				...
				$.fn.bcPaint4.update_line_status("Complete");
		},
		...
		reset_pending_line_status : function() {
				$.ajax({
		...
		**/
		onMouseUp : function(e) {
			...
			var selected_content_id = $('#bcPaint4').find('.selected_content_type').attr('id');
			content_type.push(selected_content_id);
			
			if (counter.length ==0) {counter.push(1);} else {counter.push(counter[counter.length-1]+1);}
			
			var line_text = $('#bcPaint4').find('#word_text').val();
			//alert(line_text);
			line_text = line_text.replace("  "," ");
			line_text = line_text.split(" ");
		if((jQuery.type(line_text[counter.length-1]) == "undefined" || line_text[counter.length-1] == "" ) && x_start.length <1)
				{alert("Please enter this line in input box correctly-1."); word_value.push("dummy_value");$.fn.bcPaint4.undo_function();}
			else
				{
					word_value.push(line_text[counter.length-1]);
					if (endPoint.x-startPoint.x < 0.01 * width_canvas || endPoint.y-startPoint.y < 0.01 * height_canvas) {$.fn.bcPaint4.undo_function();}
					if (word_value[counter.length-1] == "" || counter.length > line_text.length) 
						{alert("Please ensure that the line entered in the input box is correct.");$.fn.bcPaint4.undo_function();}
					$.fn.bcPaint4.outline_blocks();
				}
			},
		...
		outline_blocks : function() {
			$.fn.bcPaint4.clearCanvas();
			$('#bcPaint4').find('#bcPaint-mid-right').text("");
			var current_text ="";
			...
			jQuery.each(x_start, function( i, val ) {
				//alert(x_start[i]+"-"+y_start[i]+"-"+x_end[i]+"-"+ y_end[i]);
					...
					if(content_type[i] == 'partially_scribbled') {paintContext.strokeStyle = "blue"; $.fn.bcPaint4.reset_content_type();}
					if(content_type[i] == 'scribbled') {paintContext.strokeStyle = "green";$.fn.bcPaint4.reset_content_type();}
					if(content_type[i] == 'implied_superscript') {paintContext.strokeStyle = "purple";$.fn.bcPaint4.reset_content_type();}
					if(content_type[i] == 'indicated_superscript') {paintContext.strokeStyle = "orange";$.fn.bcPaint4.reset_content_type();}				
		...
		reset_content_type : function(){
			$('#bcPaint4').find('#bcPaint-header').find('div').removeClass('selected_content_type');
			$('#bcPaint4').find('#bcPaint-header').find('#normal').addClass('selected_content_type');
		},
